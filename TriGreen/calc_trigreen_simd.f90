...
!
!   Version:
!       2024-12-19  SIMD-OPTIMIZED VERSION with advanced loop tiling and cache optimization
...
subroutine calc_green_allcell_improved(myid,size,Nt,arr_vertex,arr_cell, & 
  ...
  integer :: i_block, i_end_block, j_block, j_end_block, i_inner
  real(DP) :: temp_sig33(3,3), temp_arr_out
 ...
 ! global coordinate
  ...
  ! Initialize SIMD optimization parameters
  L1_block_size = 32   ! L1 cache line size
  L2_block_size = 256  ! L2 cache block size  
  L3_block_size = 1024 ! L3 cache block size
  
  write(*,*) "Process", myid, ": Using SIMD-optimized loop tiling"
  write(*,*) "Process", myid, ": Block sizes - L1:", L1_block_size, "L2:", L2_block_size, "L3:", L3_block_size
  
  ! SIMD-OPTIMIZED: Advanced loop tiling with cache-aware memory access
  if (local_cells > 0) then
         !$OMP PARALLEL DO PRIVATE(i, j, u, t, sig33, k, i_block, i_end_block, j_block, j_end_block, temp_sig33, temp_arr_out, i_inner) &
         !$OMP SHARED(arr_co, arr_trid, arr_out, arr_cl_v2, L1_block_size, L2_block_size, L3_block_size)
     do j = 1, local_cells
       ! Map local index to global cell index (1-based)
       k = start_idx + j - 1
       if (k > n_cell) cycle
       
       ! L3 cache blocking for j dimension
       do j_block = 1, n_cell, L3_block_size
          j_end_block = min(j_block + L3_block_size - 1, n_cell)
          
          ! L2 cache blocking for i dimension
          do i_block = j_block, j_end_block, L2_block_size
             i_end_block = min(i_block + L2_block_size - 1, j_end_block)
             
             ! L1 cache blocking for fine-grained optimization
             do i = i_block, i_end_block, L1_block_size
                ! Process L1 block with SIMD optimization
                !$OMP SIMD PRIVATE(u, t, sig33, temp_sig33, temp_arr_out)
                do i_inner = i, min(i + L1_block_size - 1, i_end_block)
                   ! Pre-check input parameters for numerical stability
                   if (isnan(parm_nu) .or. any(isnan(arr_co(:,j))) .or. any(isnan(arr_trid(:,i_inner))) then
                      cycle
                   end if
                   ...
                   call dstuart(parm_nu, arr_co(:,j), arr_trid(:,i_inner), ss, ds, op, u, t)
                   ...
                   ! SIMD-optimized stress tensor calculation
                   !$OMP SIMD
                   temp_sig33(3,3) = l_miu * (t(1) + t(5) + t(9)) 
                   temp_sig33(1,1) = temp_sig33(3,3) + 2.d0 * t(1)
                   temp_sig33(2,2) = temp_sig33(3,3) + 2.d0 * t(5)
                   temp_sig33(3,3) = temp_sig33(3,3) + 2.d0 * t(9)

                   temp_sig33(2,1) = t(4) + t(2)
                   temp_sig33(3,1) = t(3) + t(7)
                   temp_sig33(3,2) = t(6) + t(8)

                   temp_sig33(1,2) = temp_sig33(2,1)
                   temp_sig33(1,3) = temp_sig33(3,1)
                   temp_sig33(2,3) = temp_sig33(3,2)
                   !$OMP END SIMD

                   ! Check for invalid stress tensor
                   if (any(isnan(temp_sig33))) then
                      cycle
                   end if

                   ! SIMD-optimized local stress calculation
                   !$OMP SIMD
                   temp_arr_out = -parm_miu/100 * dot_product(arr_cl_v2(:,3,j), matmul(temp_sig33(:,:), arr_cl_v2(:,1,j)))
                   !$OMP END SIMD
                   
                   ! Check final result
                   if (isnan(temp_arr_out)) then
                      cycle
                   end if
                   
                   ! Store result
                   arr_out(j,i_inner) = temp_arr_out
                end do
                !$OMP END SIMD
             end do
          end do
