...
program p_calc_green
    ...
    use mpi
   implicit none
  character(*),parameter        :: fname="fault_bp7_h10.gts"
...
!    a_op(j) = op
  ...
  do j=(myid)*Nt+1,(myid)*Nt+Nt
    ...
    do i=1, n_cell
      ...
      arr_out2(j-(myid)*Nt,i) = parm_miu/100 * dot_product(arr_cl_v2(:,3,j),matmul(sig33(:,:),arr_cl_v2(:,2,j)))
...
if(myid==0)then
 ...
 write(*,*) cTemp
  ...
  open(13,file='dsGreen_'//trim(adjustl(cTemp))//'.bin', form='unformatted',access='stream')
