...
program main
  ...
  real (DP) :: accuracy,areatot, epsv,dt_try, dt,dtmin,dt_did,dt_next,dip, &
       ...
       xilock1,xilock2,xilock3,xilock4,xi_dummy,x_dummy,z_dummy,stiff_dummy,help
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(myid==master)then
     CALL resdep(Nt_all,dip,hnucl,sigmadiff,sefffix,Lffix, &
           ...
           xilock1,xilock2,xilock3,xilock4,cca_all,ccb_all,xLf_all,&
  ...
  if(IDin.eq.0)then 
     do j=1,Nt
       ! yt(2*j-1)=0.8*(Vpl)
        yt(2*j-1)=0.8*(0.0474d0*x(i)+36.2065) 
...
!!!!!! locked portion of the fault
       ...
       do i=1,Nt
          zzfric(i) = 0.0
         ! zz(i)=yt(2*i-1)-Vpl
          zz(i)=yt(2*i-1)-0.0474*x(i)-36.2065
!          if (x(i) .lt. 50)then    ! minimum depth in mesh is 2.39 km
!              if (zz_all(i) .lt. 27)then
!                  zz(i) = 0
             ! end if                  
         ! end if
       end do
...
!----------------------------------------------------------------------------
    ...
    subroutine resdep(Nt_all,dip,hnucl,sigmadiff,sefffix,Lffix,Iperb,&
      ...
      integer :: ii,k,i,j,jj,kk,Iperb,record,l,m,nn,Nt,Nt_all

      real (DP) :: temp(DN),dep(DN),dist(DN),ptemp(Nt_all), &
           ccabmin(Nt_all),xLfmin(Nt_all),xilock1,xilock2, & 
           hnucl,sigmadiff,sefffix,Lffix,dip,xilock3,xilock4,&
      ...
      real (DP) :: slope(2), binter(2)

      real (DP) :: newtemp(4),ylock(4),xlock(4)

      factor = 1.0  !initial value of perturbation 

      do i = 1,Nt_all
         seff_all(i) = 150.d0*(z_all(i)-2.0d0)
      ...
      slope(1)=(314-250)/(xilock2-xilock4) !slope of line that connects points
      binter(1)=250-slope(1)*xilock4       !intercept of that line
      
      slope(2) = (314-250)/(xilock1-xilock3) !slope of line that connects points
      binter(2)=314-slope(2)*xilock1  
...
!!!!!!!!! Change of a- b along strike
 
         do k=1,Nt_all

            do jj=1,3
            if(x_all(k) .gt. ylock(jj) .and. x_all(k) .le. ylock(jj+1))then
               tpr(3)=newtemp(jj)+(x_all(k)-ylock(jj))*(newtemp(jj+1)-newtemp(jj))/(ylock(jj+1)-ylock(jj))
            end if
            end do
...
!!! set SSE depth effective normal stress and Dc
     do i=1,Nt_all
         if(x_all(i).ge.314.0)then
            if(z_all(i).ge.xilock1.and.z_all(i).le.xilock2)then
               seff_all(i) = sefffix
               xLf_all(i) = Lffix
            end if
         else if (x_all(i).le.250.0)then
            if(z_all(i).ge.xilock3.and.z_all(i).le.xilock4)then
               seff_all(i) = sefffix
               xLf_all(i) = Lffix
         !   else if (z_all(i) .lt. xilock3)then
         !      seff_all(i) = 10.0
         !      xLf_all(i)=abs(ccab_all(i)**2)*seff_all(i)*hnucl*(1-xnu)/(gamma*xmu*abs(ccb_all(i)))
            end if
         end if
      end do
!! correcting for kink of SSE depths along strike


!      slope(1)=(300-230)/(xilock2-xilock4) !slope of line that connects points (180,xilock4) and (280,xilock2)
!      binter(1)=180-slope(1)*xilock4       !intercept of that line

!      slope(2) = (280-230)/(xilock1-xilock3) !slope of line that connects points (180,xilock3) and (280,xilock1)
!      binter(2)=280-slope(2)*xilock1       !intercept of that line

      do i=1,Nt_all
         if(x_all(i) .gt. 250.0 .and. x_all(i) .lt. 314.0) then
            if (z_all(i) .gt. xilock1 .and. z_all(i) .lt. xilock2)then
                if(z_all(i) .gt. xilock4)then
                   if (x_all(i) .ge. (slope(1)*z_all(i)+binter(1)))then ! for xilock2 = 18 km
                       seff_all(i) = sefffix
                       xLf_all(i) = Lffix
                   end if
                else if (z_all(i) .lt. xilock3)then
                   if (x_all(i) .ge. (slope(2)*z_all(i)+binter(2)))then
                       seff_all(i) = sefffix
                       xLf_all(i) = Lffix
                   end if
                else if (z_all(i) .lt. xilock4 .and. z_all(i) .gt. xilock3)then
                       seff_all(i) = sefffix
                       xLf_all(i) = Lffix
                end if
            end if
...
!!!! add perturbation and buffer zone at both ends.
      ...
      if(Iperb.eq.1)then    !apply small perturbations in a,b
         do k=1,Nt_all
            ...
            if(x_all(k).gt.475.0.and.x_all(k).lt.510.0)then !extent of buffer
               factor = factor1
            else
               factor = 1.0
            end if

            if (x_all(k).gt.0.0.and.x_all(k).lt.50.0)then
                if (z_all(k) .gt. 0.0 .and. z_all(k) .lt. 35.0) then
                    cca_all(k) = 0.027
                    ccab_all(k) = -0.003
                    xLf_all(k) = Lffix*(sigmadiff/sefffix)
                    seff_all(k) = sigmadiff
                else
                    factor = factor1
                end if
            end if
            ...
            if(factor.eq.factor1)then
               cca_all(k)=0.01
               ccab_all(k)=0.0035 ! bufferzone original = 0.0035
