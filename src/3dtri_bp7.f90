...
program main
  ...
  real (DP), DIMENSION(:), ALLOCATABLE :: maxv,momVW,moment,&
  ...
  real(DP) :: rnuc,g1,g2
  real(DP),dimension(:),allocatable:: tau_d
  ...
  open(12,file='./parameter_bp7.txt',form='formatted',status='old')
  ...
  read(12,*)s1(1),s1(2),s1(3),s1(4),s1(5),s1(6),s1(7),s1(8),s1(9),s1(10),s1(11),s1(12),s1(13)
...
!!! modify output number
     ...
     allocate(momVW(nmv),moment(nmv),Trup(Nt_all),rup(Nt_all),area(Nt_all))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  open(6, file=trim(stiffname)//'dsGreen_'//trim(adjustl(cTemp))//'.bin',form='unformatted',access='stream')
...
if(myid==master)then
  ...
  if(myid==master)then
      open(311,file=trim(foldername)//'fltst_strk+000dp+000'//jobname,access='append',status='unknown')
      open(312,file=trim(foldername)//'fltst_strk-100dp+000'//jobname,access='append',status='unknown')
      open(313,file=trim(foldername)//'fltst_strk+000dp+100'//jobname,access='append',status='unknown')
      open(314,file=trim(foldername)//'fltst_strk+100dp+000'//jobname,access='append',status='unknown')
      open(315,file=trim(foldername)//'fltst_strk+000dp-100'//jobname,access='append',status='unknown')
      open(316,file=trim(foldername)//'fltst_strk-100dp-100'//jobname,access='append',status='unknown')
      open(317,file=trim(foldername)//'fltst_strk-100dp+100'//jobname,access='append',status='unknown')
      open(318,file=trim(foldername)//'fltst_strk+100dp-100'//jobname,access='append',status='unknown')
      open(319,file=trim(foldername)//'fltst_strk+100dp+100'//jobname,access='append',status='unknown')
      open(320,file=trim(foldername)//'fltst_strk-300dp+000'//jobname,access='append',status='unknown')
      open(321,file=trim(foldername)//'fltst_strk+000dp+300'//jobname,access='append',status='unknown')
      open(322,file=trim(foldername)//'fltst_strk+300dp+000'//jobname,access='append',status='unknown')
      open(323,file=trim(foldername)//'fltst_strk+000dp-300'//jobname,access='append',status='unknown')

       do i=311,323
   		write(i,100)'# This is the file header'
		write(i,100)'# problem=SEAS BP7-QD'
		write(i,100)'# author=D.Li'
        write(i,100)'# code=TriBIE'
		write(i,100)'# date=2022/8/1'
		write(i,100)'# element_size = 10 m'
 ...
 100	format(A)
       ...
       open(30,file=trim(foldername)//'maxvall'//jobname,access='append',status='unknown')
        ...
        write(i,100)'# Column #4 = moment rate of VW(N-m/s)'
        write(i,100)'# The line below lists the names of the data fields:'
        write(i,'(A,1x,A,1x,A,1x,A)')'t','max_slip_rate','moment_rate','moment_rate_VW'
        write(i,100)'# Below is the time-series data.'  
      ...
      open(401,file=trim(foldername)//'blkst_strk+000fn+200dp+000'//jobname,access='append',status='unknown')
      open(402,file=trim(foldername)//'blkst_strk+000fn+400dp+000'//jobname,access='append',status='unknown')
      open(403,file=trim(foldername)//'blkst_strk-300fn+400dp+000'//jobname,access='append',status='unknown')
      open(404,file=trim(foldername)//'blkst_strk+000fn+400dp+300'//jobname,access='append',status='unknown')
      open(405,file=trim(foldername)//'blkst_strk+300fn+400dp+000'//jobname,access='append',status='unknown')
      open(406,file=trim(foldername)//'blkst_strk+000fn+400dp-300'//jobname,access='append',status='unknown')
    do i = 401,406
        write(i,100)'# This is the file header'
        write(i,100)'# problem=SEAS BP7-QD'
        write(i,100)'# author=D.Li'
...
122 format(100(E15.7,1X))
      write(501,122) 0d0,0d0, z_all(pdp(:)) !meter 
      write(502,122) 0d0,0d0, x_all(pstrk(:))
      write(503,122) 0d0,0d0, z_all(pdp(:))
      write(504,122) 0d0,0d0, x_all(pstrk(:))
...
!        if(vi(j).gt.1e-4) yt(2*j-1)=3*vi(j)
  ...
  do while(cyclecont) 
     ...
     if(t*yrs.gt.T0)then
     	...
     	dt = dt_did
     	dt_try = dt_next
    else !! changes: fix time step of every 0.02 sec in the first 1.0 sec for nucleation
      dt_try= 1.d-12 !aging law 1d-11
      t = t+ dt_try
      ...
      do j=1,2*Nt
        yt_scale(j)=dabs(yt(j))+dabs(dt_try*dydt(j))
        yt0(j) = yt(j)
        yt(j) = yt_scale(j)  !! need to update yt(j) !!!!!!!!!!!!!!!!!!!
      end do
      dt = dt_try
    end if
     ...
     do i=1,Nt
        ...
        tau1(i) = seff(i)*cca(i)*dlog(help+dsqrt(1+help**2))*phy1(i)
        tau2(i) = seff(i)*cca(i)*dlog(help+dsqrt(1+help**2))*phy2(i)
        !phy1(i) = tau1(i)/dsqrt(tau1(i)**2+tau2(i)**2)
        !phy2(i) = tau2(i)/dsqrt(tau1(i)**2+tau2(i)**2)
        !tau1(i) = phy1(i)*tautmp
        !tau2(i) = phy2(i)*tautmp

        slipinc(i) = 0.5*(yt0(2*i-1)+yt(2*i-1))*dt*phy1(i)
        slipdsinc(i)=0.5*(yt0(2*i-1)+yt(2*i-1))*dt*phy2(i)
     ...
     if(myid==master)then
        ...
        do i=1,Nt_all      !!!!!!!!!!!!!!!!! find max velocity
          ...
          if(cca_all(i)-ccb_all(i).le.0.0)then
           momVW(imv) = momVW(imv)+0.5*(yt0_all(2*i-1)+yt_all(2*i-1))/yrs*1d-3*area(i)*xmu*1d6*1d5
          end if
          if(dsqrt((x(i)-y2)**2+(z(i)-y3)**2).lt.1.5*200.0) then
...
!!!!!  SEAS output variables
      ...
      do i = 1,n_obv
       ...
       obvs(imv,3,i) = -disp3/1d3
		...
		write(*,130) t,dlog10(maxv(imv)*1d-3/yrs),moment(imv),momVW(imv)

 130    format(E22.15,3(1X,E15.7))
     ...
     if(myid==master)then 
        ...
        call output(Ioutput,Isnapshot,Nt_all,Nt,inul,imv,ias,icos,isse,x,&
             tmv,tas,tcos,tnul,tsse,maxv,momVW,moment,outs1,maxnum,msse1,msse2, areasse1,areasse2,&
 ...
 if(myid==master)then
       ...
       do j=1,Nt_all
          write(i,111) x_all(j),z_all(j),Trup(j)
        end do
        close(i)
...
if(myid==master)then 
     ...
     Ioutput = 1
     call output(Ioutput,Isnapshot,Nt_all,Nt,inul,imv,ias,icos,isse,x,&
          tmv,tas,tcos,tnul,tsse,maxv,momVW,moment,outs1, &
  ...
  if(myid==master)then 
     DEALLOCATE (x_all,xi_all,yt_all,dydt_all,yt_scale_all,yt0_all,&
          ...
          maxnum,maxv,momVW,moment,outs1,&
     ...
     deallocate(surf1,surf2,surf3,Trup,rup,area,obvs)
     deallocate(pstrk,pdp,obvstrk,obvdp)
  end if

  DEALLOCATE (phy1,phy2,islip1,islip2,itran1,itran2,zzfric,zzfric2,&
       x,z,z_all,xi,cca,ccb,seff,&
       xLf,tau1,tau2,tau0,slipds,slipdsinc,slip,slipinc, &
       yt,dydt,yt_scale,yt0,sr,vi)
  deallocate (tau_d,stiff,stiff2)
...
!------------------------------------------------------------------------------
     subroutine derivs(myid,dydt,nv,Nt_all,Nt,t,yt,z_all,x)
       USE mpi
       USE phy3d_module_non, only: yrs, tsec,phy1,phy2,tau1,tau2, stiff2,stiff,cca,ccb,seff,xLf,eta,f0,Vpl,V0,Lratio,nprocs,&
            tm1,tm2,tmday,tmelse,tmmidn,tmmult,T0,y2,y3,tau_p0
       implicit none
       integer, parameter :: DP = kind(1.0d0)
       ...
       real (DP) :: SECNDS,rnuc,g11,g22
       real (DP) :: tau_inc(Nt),sr(Nt),z_all(Nt_all),x(Nt),zz(Nt),zz_ds(Nt),zzfric(Nt),zz_ds_all(Nt_all),zz_all(Nt_all),zzfric2(Nt)
       intrinsic imag,real
       ...
       do i=1,Nt
         ...
         if (abs(x(i)).gt.400.0.or.abs(z_all(myid*Nt+i)).gt.400.0) then
             zz(i) = 0.0
             yt(2*i-1)=Vpl
             zz_ds(i)=0.0
         end if
...
!             zzfric2(i)=zzfric2(i) - stiff2(i,j)*zz_ds_all(j) ! dip-

          end do
      tau_inc(i)=0.0d0
       if(t*yrs.gt.0.d0) then  ! to unify time in second 
        if(t*yrs.lt.T0) then
          tsec = t*yrs
          g22 = dexp((tsec-T0)**2.0/tsec/(tsec-2.0*T0))*(-2.0*T0*T0*(tsec-T0)/tsec**2.0/(tsec-2.0*T0)**2.0)
        else
          g22 = 0.d0
        end if
         rnuc = dsqrt((x(i)-y2)**2.0+(z_all(myid*Nt+i)-y3)**2.0)
        if(rnuc.lt.150.d0) then ! Rnuc
          g11 = dexp(rnuc**2.0/(rnuc**2.0-150.0**2.0))
         else
          g11 = 0.d0
         end if
         tau_inc(i) = g11*g22*tau_p0*yrs  ! times yrs to unify stress in integeral !!!!!!!!!!! 
        end if
       end do
...
!slip law	     
          deriv3 = -yt(2*i-1)*yt(2*i)/xLf(i)*dlog(yt(2*i-1)*yt(2*i)/xLf(i))
          dydt(2*i-1) = -(-tau_inc(i)+zzfric(i)+deriv1*deriv3)/(eta+deriv2) ! total shear traction
...
real (DP) :: x(Nt),maxnum(nmv),momVW(nmv),moment(nmv),maxv(nmv),outs1(nmv,7,10),&
...
if(Ioutput == 0)then    !output during run 
   ...
   if(imv==nmv)then
      ...
      do i=1,nmv
         write(30,130)tmv(i),dlog10(maxv(i)*1d-3/yrs),moment(i),momVW(i)
        do j=311,323
...
144 format(E22.14,100(1X,E15.7))
      do j=401,406
         close(j)
      end do
