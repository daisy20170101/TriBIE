...
program main
  ...
  integer ::Nt_all, Nt,ndt,ndtnext,kk,ii,n,l,ndt_v,ndt_inter,Itout,&  
       ...
       Iperb,record,Isnapshot,record_cor,n_source,n_total,s1,s2,s3,s4,s5,s6,s7,s8
  ...
  open(12,file='./parameter2.txt',form='formatted',status='old')
...
!!!modified data read in
  read(12,*)Nab,n_total,n_source,Nt,Lratio,nprocs,hnucl
  ...
  read(12,*)tstart1,tend1,tstart2,tend2,tstart3,tend3	
...
110 format(A)
  ...
  !Nltot = Nl*Lratio   !Total element along strike,Nl is where R-S friction applies
  !num. of rows each proc. 

  if(mod(n_source,nprocs)/=0)then
     write(*,*)'Nd_all must be integer*nprocs. Change nprocs!'
     STOP
  else
     write(*,*)'Each cpu calculates',n_source/nprocs,'cells'
  end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ALLOCATE (islip1(Nl),islip2(Nl),itran1(Nl),itran2(Nl), &
  ...
  ALLOCATE (stiff(Nt,n_total))   !!! stiffness of Stuart green calculation
  ...
  do i=1,Nt !! observe
     do j=1,n_total !! source
        read(5) stiff(i,j)

        if(stiff(i,j).lt.-0.3d0.or.stiff(i,j).gt.0.8d0)then
           stiff(i,j) = 0.d0
  ...
  call MPI_Bcast(islip1,Nl,MPI_integer,master,MPI_COMM_WORLD,ierr)
  call MPI_Bcast(islip2,Nl,MPI_integer,master,MPI_COMM_WORLD,ierr)
  call MPI_Bcast(itran1,Nl,MPI_integer,master,MPI_COMM_WORLD,ierr)
  call MPI_Bcast(itran2,Nl,MPI_integer,master,MPI_COMM_WORLD,ierr)
  ...
  do while(cyclecont) 
     ...
     call derivs(myid,dydt,2*Nt,Nt_all,Nt,t,yt,n_total,n_source,z_all,x) 

     do j=1,2*Nt
        yt_scale(j)=dabs(yt(j))+dabs(dt_try*dydt(j))
     end do
     CALL rkqs(myid,yt,dydt,2*Nt,Nt_all,Nt,t,dt_try,accuracy,yt_scale, &
          dt_did,dt_next,n_total,n_source,z_all,x)
     ...
     if(myid==master)then
        ...
        do i=1,Nt_all      !!!!!!!!!!!!!!!!! find max velocity
           ...
           if(i.eq.s1)then
              maxv_s1(imv) = yt_all(2*i-1)
           else if(i.eq.s2)then
              maxv_s2(imv) = yt_all(2*i-1)
           else if(i.eq.s3)then
              maxv_s3(imv) = yt_all(2*i-1)
           else if(i.eq.s4)then
              maxv_s4(imv) = yt_all(2*i-1)
           else if(i.eq.s5)then
              maxv_s5(imv) = yt_all(2*i-1)
           else if(i.eq.s6)then
              maxv_s6(imv) = yt_all(2*i-1)
           else if(i.eq.s7)then
              maxv_s7(imv) = yt_all(2*i-1)
           else if(i.eq.s8)then
              maxv_s8(imv) = yt_all(2*i-1)
           end if

        end do
!!!!!
        if(tmv(imv).ge.3.00.and.tmv(imv).lt.4.0)then
           write(*,*) tmv(imv)
           open(71,file=trim(foldername)//'checkz1'//jobname,status='unknown')
           open(72,file=trim(foldername)//'checkz2'//jobname,status='unknown')
           do i=1,Nt_all
              write(71,*)  dlog10(yt_all(2*i-1))-3
              write(72,*)  yt_all(2*i)
           end do
           close(71)
           close(72)
        ...
        if(((t.ge.tstart1).and.(t.le.tend1)).or.((t.ge.tstart2).and.(t.le.tend2)).or.((t.ge.tstart3).and.(t.le.tend3)))then
           i_nul = i_nul + 1
           if(i_nul.ge.n_nul_int)then
              inul = inul +1 
              tnul(inul) = t
              do k=1,Nt_all
                 v_nul(k,inul)=yt_all(2*k-1)
                 slip_nul(k,inul)=slip_all(k)*(1.d-3)
              end do
              i_nul = 0
           end if
        end if
        ...
        if(t.ge.tssestart.and.t.le.tsseend)then
           ...
           if((maxv(imv)/yrs).le.vcos.and.maxv(imv).ge.vsse1)then
              ...
              if(tslipsse.ge.0.002739726)then  
...
!!! find the total number of output elements and cell number of specific depth.
     ...
     if(myid==master)then 
        Ioutput = 0 
        call output(Ioutput,Isnapshot,Nt_all,Nt,hd,inul,imv,ias,icos,isse,x,&
             tmv,tas,tcos,tnul,tsse,maxv,maxv_s1,maxv_s2,maxv_s3,maxv_s4,maxv_s5,maxv_s6,&
             maxv_s7,maxv_s8,maxnum,msse1,msse2, areasse1,areasse2,&
             ...
             n_intz1,n_intz2,n_intz3,slipz1_v)         
  ...
  if(myid==master)then 
     ...
     Ioutput = 1
     call output(Ioutput,Isnapshot,Nt_all,Nt,hd,inul,imv,ias,icos,isse,x,&
          tmv,tas,tcos,tnul,tsse,maxv,maxv_s1,maxv_s2,maxv_s3,maxv_s4,maxv_s5,maxv_s6,maxv_s7,maxv_s8, &
  ...
  DEALLOCATE (stiff)
  DEALLOCATE (islip1,islip2,itran1,itran2,x,z_all,xi,yt,dydt,yt_scale,tau,tau0,slip,slipinc)
...
!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
subroutine rkqs(myid,y,dydx,n,Nt_all,Nt,x,htry,eps,yscal,hdid,hnext,n_total,n_source,z_all,p)
  Use mpi
  USE phy3d_module_non, only : nprocs
  implicit none
  integer, parameter :: DP = kind(1.0d0)   
  integer :: n,i,j,k,NMAX,Nt,Nt_all,n_total,n_source
...
1 call rkck(myid,dydx,h,n,Nt_all,Nt,y,yerr,ytemp,x,derivs,n_total,n_source,z_all,p)
...
!-----------------------------------------------------------------------------
!-----------------------------------------------------------------------------
     subroutine rkck(myid,dydx,h,n,Nt_all,Nt,y,yerr,yout,x,derivs,n_total,n_source,z_all,p)
       USE phy3d_module_non, only :nprocs
       implicit none
       integer, parameter :: DP = kind(1.0d0)   
       integer :: n,i,NMAX,myid,Nt_all,Nt,n_total,n_source
       external derivs
       ...
       call derivs(myid,ak2,n,Nt_all,Nt,x+A2*h,ytemp,n_total,n_source,z_all,p)
       do  i=1,n
          ytemp(i)=y(i)+h*(B31*dydx(i)+B32*ak2(i))
       end do
       call derivs(myid,ak3,n,Nt_all,Nt,x+A3*h,ytemp,n_total,n_source,z_all,p)
       do  i=1,n
          ytemp(i)=y(i)+h*(B41*dydx(i)+B42*ak2(i)+B43*ak3(i))
       end do
       call derivs(myid,ak4,n,Nt_all,Nt,x+A4*h,ytemp,n_total,n_source,z_all,p)
       do  i=1,n
          ytemp(i)=y(i)+h*(B51*dydx(i)+B52*ak2(i)+B53*ak3(i)+  &
               B54*ak4(i))
       end do
       call derivs(myid,ak5,n,Nt_all,Nt,x+A5*h,ytemp,n_total,n_source,z_all,p)
       do  i=1,n
          ytemp(i)=y(i)+h*(B61*dydx(i)+B62*ak2(i)+B63*ak3(i)+  &
               B64*ak4(i)+B65*ak5(i))
       end do
       call derivs(myid,ak6,n,Nt_all,Nt,x+A6*h,ytemp,n_total,n_source,z_all,p)
       do  i=1,n
...
!------------------------------------------------------------------------------
!------------------------------------------------------------------------------
     subroutine derivs(myid,dydt,nv,Nt_all,Nt,t,yt,n_total,n_source,z_all,x)
       USE mpi
       USE phy3d_module_non, only: stiff,cca,ccb,seff,xLf,eta,f0,Vpl,V0,Lratio,nprocs,&
            tm1,tm2,tmday,tmelse,tmmidn,tmmult
       implicit none
       integer, parameter :: DP = kind(1.0d0)
       integer :: nv,n,i,j,k,kk,l,ii,Nt,Nt_all,n_total,n_source
       real (DP) :: t,yt(nv),dydt(nv)   
       real (DP) :: deriv3,deriv2,deriv1,small
       real (DP) :: psi,help1,help2,help
       real (DP) :: SECNDS
       real (DP) :: z_all(Nt_all),x(Nt),zz(Nt),zzfric(Nt),zzfric_all(n_total),zz_all2(n_total),zz_all(Nt_all)
       intrinsic imag,real
...
!!!! add the locked zone above 20 km for the seismogenic zone
       ...
       do j=1,n_source
          if(z_all(j).gt.19.5)then
             zz_all2(j+n_total-n_source) = zz_all(j)
          else
             zz_all2(j+n_total-n_source) = 0
          end if
       end do

       do j=1,n_total-n_source
          zz_all2(j) = -Vpl
       end do
!!!!!!!!!!!!!!!!!!!!
       ...
       do i=1,Nt
          do j=1, n_total
             ...
             zzfric(i)=zzfric(i) + stiff(i,j)*zz_all2(j)
          end do
       end do

 !      do i=1,1
          !write(*,'(1x,I6,E23.13)') myid*Nt+i,zzfric(i)
 !      end do
...
!----------------------------------------------------------------------------
    ...
    subroutine resdep(Nt_all,dip,hnucl,sigmadiff,sefffix,Lffix, &
      ...
      USE mpi
      USE phy3d_module_non, only: p18,Nl,Nd,Nab,xmu,xnu,gamma, &
      ...
      real (DP) :: temp(DN),dep(DN),dist(DN),ptemp(Nt_all), &
           ...
           hnucl,hl,hd,sigmadiff,sefffix,Lffix,dip, &
...
!!!! add perturbation and buffer zone at both ends.
      ...
      if(Iperb.eq.1)then    !apply small perturbations in a,b
         do k=1,Nt_all
            ...
            if(x_all(k).gt.80.0.and.x_all(k).lt.120.0)then
               factor = factor1
            else if(x_all(k).gt.-390.0.and.x_all(k).lt.-330.0)then
               factor = factor1
            else
               factor = 1.0
            end if
...
!------Output -------------------------------------------
!--------------------------------------------------------
subroutine output(Ioutput,Isnapshot,Nt_all,Nt,hd,inul,imv,ias,icos,isse,x,&
    tmv,tas,tcos,tnul,tsse,maxv,maxv_s1,maxv_s2,maxv_s3,maxv_s4,maxv_s5,maxv_s6,&
    maxv_s7,maxv_s8,& 
    maxnum,msse1,msse2,areasse1,areasse2, &
     slipz1_inter,slipz1_tau,slipz1_sse,&
...
real (DP) :: x(Nt),maxnum(nmv),maxv(nmv),maxv_s1(nmv),maxv_s2(nmv),maxv_s3(nmv),maxv_s4(nmv),maxv_s5(nmv),&
        msse1(nsse),msse2(nsse),areasse1(nsse),areasse2(nsse),maxv_s6(nmv),maxv_s7(nmv),maxv_s8(nmv), &
	tmv(nmv),tas(nas),tcos(ncos),tnul(nnul),tsse(nsse)
